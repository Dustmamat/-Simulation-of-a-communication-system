...
constellation_points = [ ...
    R_outer_star 0; 0 R_outer_star; -R_outer_star 0; 0 -R_outer_star; ... 
    R_inner_star*cos(pi/4) R_inner_star*sin(pi/4); R_inner_star*cos(3*pi/4) R_inner_star*sin(3*pi/4); ...
    R_inner_star*cos(5*pi/4) R_inner_star*sin(5*pi/4); R_inner_star*cos(7*pi/4) R_inner_star*sin(7*pi/4) ... 
];
constellation_name = 'Star 8-QAM';
...
% Look up I and Q values from the normalized constellation
...
figure;
Fs = SpS;
NFFT = 2^nextpow2(length(tx_complex_envelope)); % Next power of 2 for FFT
Y = fftshift(fft(tx_complex_envelope, NFFT));
deltaFrequency = Fs/(NFFT);
f = (-Fs/2:deltaFrequency:Fs/2-deltaFrequency);

plot(f, 20*log10(abs(Y)));
title(sprintf('Spectrum of Generated %s Complex Envelope Signal', constellation_name));
...
fprintf('Simulating BER vs. Eb/N0...\n');
H = fftshift(sinc(f));
...
for i = 1:length(EbN0_dB_range)
    ...
    rx_sampled_symbols = rx_complex_envelope(SpS/2:SpS:end); 
    ...
    fprintf('  Eb/N0 = %f dB, Simulated BER = %e\n', ...
        EbN0_dB, BER(i));
end
%%
figure;
semilogy(EbN0_dB_range, BER, 'o-', 'LineWidth', 1.5);
grid on;
xlabel('Eb/N0 (dB)');
...
xlabel('In-Phase (I)');
ylabel('Quadrature (Q)');
legend('Constellation Points', 'Full Trajectory', 'Optimal Sampled Points','FontSize', 6);
max_val_noiseless = max(abs(constellation_points(:))) * 1.5; 
...
fprintf('Plotting constellation at BER target of %e (approx. Eb/N0 = %f dB) for %s...\n', BER_target, EbN0_target_dB, constellation_name);
...
sigma_n_target = sqrt(sigma_n_squared_target); 
...
figure;
plot(real(rx_sampled_symbols_plot), imag(rx_sampled_symbols_plot), 'x', 'MarkerSize', 6, 'Color', [0.85 0.325 0.098]); % Received points
grid on;
axis equal;
xlabel('In-Phase (I)');
ylabel('Quadrature (Q)');
