...
% Circular 8-QAM (8-PSK): All points on a single circle.
theta = (0:M-1) * (2*pi/M);
r = 3;
constellation_points = r*[cos(theta(:)), sin(theta(:))];
constellation_name = 'Circular 8-QAM (8-PSK)';
...
for i = 1:length(EbN0_dB_range)
    ...
    theor_BER(i) =(1/k) * erfc(sqrt(k * EbN0_linear) * sin(pi/M));
...
Ber_circular = BER;
save('Bercircular.mat', 'Ber_circular');
...
xlabel('In-Phase (I)');
ylabel('Quadrature (Q)');
max_val_noisy = max(abs(constellation_points(:))) * 1.5; % Dynamic limits
