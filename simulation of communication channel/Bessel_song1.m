...
%% Analyze 1-signal with disturbance
[data, Fs] = audioread('John Lennon - Imagine.mp3'); 
N= length(data);
x_ref_1 = data(1:N, 1);

load('307576_SONG1.mat');
N = length(xtot);
x_1 = xtot(1:N, 1);
...
X=fftshift(fft(x_1)); % DTF of the signal
...
freq_range = 1000:100:5000; % I used frequency range with big steps to reduce processing time but any step can be used.
